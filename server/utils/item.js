...
function itemSequentialOrderNumber(category_id, material_type_id, sub_type_id) {
    return new Promise((resolve, reject) => {
        db.executeQuery(`Select item_order From item_master where category_id=? and material_type_id=? and sub_type_id=?
        order BY item_order DESC LIMIT 1`, category_id, material_type_id, sub_type_id)
            .then(rows => {
...
function getSubTypeNumber(material_type_id) {
    return new Promise((resolve, reject) => {
        db.executeQuery(`Select sub_type_number From sub_types where material_type_id=? order BY sub_type_number DESC LIMIT 1`,
            material_type_id)
            .then(rows => {
                if (rows.length > 0) {
...
function getMaterialTypeNumber(category_id) {
    return new Promise((resolve, reject) => {
        db.executeQuery(`Select material_type_number From material_types where category_id=? order BY material_type_number DESC LIMIT 1`,
            category_id)
            .then(rows => {
                if (rows.length > 0) {
